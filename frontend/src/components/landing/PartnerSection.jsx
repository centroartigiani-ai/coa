  ...
  return (
    <section id="partner" data-testid="partner-section" className="py-24 md:py-32 px-6 md:px-12 mx-auto max-w-7xl">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div>
          <SectionHeading number="05" label="Diventa partner" title={<>Sei un artigiano? <span className="text-[#FF5A00]">Lavora con noi.</span></>} />
          <Reveal delay={0.15}>
            ...
            <ul className="mt-8 space-y-4">
              {BENEFITS.map((b, i) => (
                <li key={i} data-testid={`partner-benefit-${i}`} className="flex items-center gap-4">
                  <span className="w-7 h-7 bg-[#FF5A00]/10 border border-[#FF5A00]/30 flex items-center justify-center shrink-0">
                    <Check className="w-4 h-4 text-[#FF5A00]" strokeWidth={2.5} />
                  </span>
          ...
          <Reveal delay={0.35}>
            <button
              ...
              onClick={onPartner}
              className="group mt-10 bg-[#FF5A00] text-white px-8 py-4 font-medium inline-flex items-center gap-2 hover:bg-[#E04F00] transition-colors"
            >
        ...
        <Reveal delay={0.2}>
          <div className="relative">
            <div className="absolute -bottom-4 -right-4 w-full h-full border border-[#FF5A00]/40" />
      ...
      <div className="mt-32">
        <Reveal>
          <p className="text-xs font-bold tracking-[0.3em] uppercase text-[#FF5A00] mb-6 flex items-center gap-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-px bg-white/10 border border-white/10">
          {FLOW.map((f, i) => (
            <Reveal key={i} delay={i * 0.08} className="bg-[#0A0A0A]">
              <div data-testid={`flow-step-${i + 1}`} className="p-6 h-full bg-[#0A0A0A] hover:bg-[#111111] transition-colors duration-500">
                <span className="font-display text-sm font-black text-white/20">0{i + 1}</span>
                <f.icon className="w-5 h-5 text-[#FF5A00] mt-4" strokeWidth={1.5} />
