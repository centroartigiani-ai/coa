...
export default function HowItWorks() {
  return (
    <section id="come-funziona" data-testid="how-it-works-section" className="py-24 md:py-32 px-6 md:px-12 mx-auto max-w-7xl">
      <SectionHeading number="01" label="Come funziona" title={<>Dal problema all'artigiano, <span className="text-[#F2A93B]">in quattro mosse.</span></>} />
      <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-px bg-white/10 border border-white/10">
        {STEPS.map((s, i) => (
          <Reveal key={i} delay={i * 0.1} className="bg-[#1C1C1E]">
            <div data-testid={`step-card-${i + 1}`} className="group p-8 h-full bg-[#1C1C1E] hover:bg-[#26241F] transition-colors duration-500">
              <div className="flex items-start justify-between">
                <s.icon className="w-6 h-6 text-[#F2A93B]" strokeWidth={1.5} />
                <span className="font-display text-5xl font-black text-white/10 group-hover:text-[#F2A93B]/40 transition-colors duration-500">0{i + 1}</span>
              </div>
